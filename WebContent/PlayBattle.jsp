...
<div class="form-group">
	<div class="input-group">
		<%
			...
			HashMap<String, List<String>> domain_robot_map = new HashMap<String, List<String>>();							
				try {
					...
					//String connectionUrl = "jdbc:sqlserver://robocode.database.windows.net:1433;database=robocode;user=robocode@robo;password=robo.123;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;";

					//Connection connection = DriverManager.getConnection(connectionUrl,"groupb1","Qwerty1234");
		String connectionURL = "jdbc:sqlserver://cloudfall16.database.windows.net:1433;database=DbRobo;user=RobocodeAdmin@cloudfall16;password=admin@robocode123;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;";
									Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver").newInstance();
									Connection connection = DriverManager.getConnection(connectionURL);// "robo","robo.123");
					Statement statement = connection.createStatement();
					String selectString="SELECT userID, packageID, robotID from dbo.robot";
					resultset = statement
...
&nbsp;&nbsp;Package
         ...
         <td width="250px"><script  type="text/javascript">
							function AddtoChosen(){
								$('#displayrobots option:selected').each(function(i,val){
									// alert(val.value);
								     
									// $('#selectrobots').val(val.value);
									 $('#selectrobots').append('<option value="' + $('#package option:selected').val() +"."+ val.value +'">'+ val.value +'</option>');
